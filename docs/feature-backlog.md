...
| PWA 支持 | architecture.md | **已完成** | 离线可用、桌面安装 |
...
| 功能 | 来源 | 状态 | 说明 |
|------|------|------|------|
| WebSocket 断线自动重连 | 审查 | **已完成** | 指数退避 3s/6s/12s/max30s + 连接状态指示器 |
| PWA 支持 | architecture.md | **已完成** | manifest.json + 图标 + Apple Web App meta |
| 聊天空状态提示 | 审查 | **已完成** | 无消息/无搜索结果时显示友好文案 |
| API 限流 | 审查 | **已完成** | 内存令牌桶，120次/分钟/IP |
| 规则重建权限保护 | 审查 | **已完成** | admin_key 验证 |
| 自定义滚动条 | 审查 | **已完成** | WebKit + Firefox 暗色滚动条 |
| 移动端响应式 | 审查 | **已完成** | 768px 断点单列适配 |
| 连接管理器并发安全 | 审查 | **已完成** | broadcast 快照连接列表防并发修改 |
...
| 功能 | 来源 | 状态 | 说明 |
|------|------|------|------|
| 断线自动重连 | 本次审查 | **已完成** | WebSocket onclose 自动重连 + 指数退避 + 连接状态指示器 |
